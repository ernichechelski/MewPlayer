...
/**
 * Klasa obsługująca odtwarzanie plików formatu mp3
 * Zawiera w sobie klasę Playlist i klasę Song, które są wymagane do działania.
 * Obsługuje wiele plików, odczytywanie metadaty, mieszanie kolejności odtwarzania
 * Wymaga trzech kontrolek UI, dwóch progressBar'ów do wyświetlenia postępu odtwarzania i głośności,
 * oraz ImageView do wyświetlenia okładki. Ich specyfika wymaga ustawienia ich od początku(tzn nie wiem jak się da inaczej).
 * 
 * Wywołanie setMetaDataLabels() zapewnia wyświetlanie danych obecnie odtwarzanego utworu.
 * Wywołanie setVisualiser() zapewnia obsługę BarChart do wyświetlania struktury odtwarzanego dzwięku
 * Wywołanie addSongs() pozwala na dodanie utworów
 * Wywołanie play(), pause(), stop(), next() i previous() pozwala na sterowanie odtwarzaniem
 * 
 * Klasa Playlist zawiera wszelkie narzędzia do obługi TableView.
 * By podejrzeć jakie dane o pojedyńczym utworze są zapisywane, zajrzyj do klasy Song
...
/**
 * Enum używany do opisywania stanu obiektu klasy MediaPlayerEngine
 * 
 * */
enum Status{
...
public class MediaPlayerEngine {
    ...
    private AudioSpectrumListener audioSpectrumListener;//Klasa ta służy do obsługi wizualizacji muzyki, na bierząco udostępniając bierzące dane o strukturze dzwięku @see setVisualiser()
    private XYChart.Data<String, Number>[] series1Data;//Powiązane z audioSpectrumListener, służy do zapisu danych, które mogą być wyświetlane przez klasę Chart @see setVisualiser()
    ...
    /**
     * Konstruktor klasy 
     * @param ProgressBar pb Utworzenie interfejsu użytkownika wymaga podania istniejącego obiektu klasy ProgressBar, służącego do sterowania postępem odtwarzania utworu
     * @param ProgressBar vb Utworzenie interfejsu użytkownika wymaga podania istniejącego obiektu klasy ProgressBar, służącego do sterowania głośnością odtwarzania utworu
     * @param ImageView iv Utworzenie interfejsu użytkownika wymaga podania istniejącego obiektu klasy ImageView, służącego do wyświetlania okładki albumu obecnego utworu
     * */
        ...
        audioSpectrumListener = new AudioSpectrumListener() {
            @Override public void spectrumDataUpdate(double timestamp, double duration,
                ...
                for (int i = 0; i < series1Data.length; i++) {
                    series1Data[i].setYValue(2*magnitudes[i] + 2*60);
                }
            }
        };
     ...
     * */
    public void setVisualizer(final BarChart<String, Number> bc)
    {
        ...
        yAxis.setLabel("Articles per Year");
        ...
        for (int i=0; i<series1Data.length; i++) {
            ...
            series1Data[i] = new XYChart.Data<String,Number>(categories[i],65);
